...
module.exports = {
  ...
  execute     : async (msg, args) => {
    ...
    .promise([
      `SELECT npc_configs.npc_id, npc_configs.name, CONCAT_WS('~', drop_tables.main, drop_tables.charm) AS drops FROM json('${liveserver_configs_dir}/drop_tables.json') AS drop_tables LEFT JOIN json('${liveserver_configs_dir}/npc_configs.json') AS npc_configs ON npc_configs.id = drop_tables.npc_id WHERE npc_configs.id = "${npc_id}"`,
    ][0]);
