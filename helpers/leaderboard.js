...
const ironmanModes = {
  'NONE': { icon: '<:Yellow_partyhat:700168466027642941>', label : '', message_id: leaderboard_message_id },
  'STANDARD': { icon: '<:Ironman_chat_badge:700168465675190393>', label : '(Ironman)', message_id: leaderboard_ironman_message_id },
  'HARDCORE': { icon: '<:Hardcore_ironman_chat_badge:700168466019385495>', label : '(Hardcore Ironman)', message_id: leaderboard_hardcore_ironman_message_id },
  'ULTIMATE': { icon: '<:Ultimate_ironman_chat_badge:700168466292015104>', label : '(Ultimate Ironman)', message_id: leaderboard_ultimate_ironman_message_id },
};

async function getTop(amount = 25, skill_id = 0, ironman = 'NONE'){
  if (skill_id == 24) // Wealth
    return await connection.query(`SELECT username, netWorth AS wealth FROM members WHERE ironManMode='${ironman}' ORDER BY netWorth DESC LIMIT ${amount}`).catch(error);
  else if (skill_id >= 0) // Skill
    return await connection.query(`SELECT username, xp_${skill_id} AS xp FROM highscores WHERE ironManMode='${ironman}' ORDER BY xp_${skill_id} DESC LIMIT ${amount}`).catch(error);
  else // Overall
    return await connection.query(`SELECT username, overall_xp AS xp, total_level AS level FROM highscores WHERE ironManMode='${ironman}' ORDER BY total_level DESC, overall_xp DESC LIMIT ${amount}`).catch(error);
}

async function updateLeaderboard(guild, ironmanMode = 'NONE', reaction = 'null'){
  ...
  const leaderboard_message = await leaderboard_channel.fetchMessage(ironmanModes[ironmanMode].message_id).catch(O_o => {});
  ...
  const output = [
    `${ironmanModes[ironmanMode].icon || ironmanModes.NONE.icon} __***Top ${results.length} ${skills[skill_id] ? skills[skill_id].name : 'Overall'}:***__ ${ironmanModes[ironmanMode].label || ''}`,
    ...results.map((res, place) => `**#${place + 1}** \`${res.wealth != null ? RuneScape.formatMoney(res.wealth).padStart(6, ' ') : `level ${skills[skill_id] ? RuneScape.xp_to_level(res.xp).toString().padEnd(3, ' ') : res.level.toString().padEnd(4, ' ')} (${RuneScape.formatNumber(res.xp)} xp)`}\` **${res.username}**`),
    '',
...
async function listenToLeaderboardReactions(guild, ironmanMode, leaderboard_message){
  const filter = (reaction, user) => !user.bot;
  leaderboard_message.awaitReactions(filter, { max: 1 })
    .then(collected => {
