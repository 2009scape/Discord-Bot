...
client.once('ready', async() => {
  ...
  client.guilds.forEach(guild=>{
    ...
    new RunOnInterval(6e4 /* 1 Minute */, async ()=>{
      const results = await connection.query('SELECT COUNT(*) AS players FROM `members` WHERE online=1').catch(error);
      const players = results[0] ? results[0].players : 0;
      client.user.setActivity(`${players} Player${players != 1 ? 's' : ''} Online`);
    }, true);
  });
});
...
function login(token, attempt = 1){
  ...
  client.login(token).catch(e=>{
    if (e.code == 'ETIMEDOUT'){
      ...
      error(`Login timed out (${attempt}), retrying in ${minutes} minute${minutes > 1 ? 's' : ''}...`);
      setTimeout(()=>{
        login(token, ++attempt);
      }, minutes * 6e4);
    }
